...
{
public:	
    String        (const int size = default_size); // Initialize Sting to '\0', w/ optional capacity
    ...
    int getLength() const { return length; }; // returns the length of a string
    ...
    std::vector<String> split(); // splits a string on a char and returns it as a vector
    ...
    String getline(std::istream& in); // gets a line
